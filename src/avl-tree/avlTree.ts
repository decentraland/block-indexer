 ...
 */
export interface AvlTreeApi<K, V> {
  /**
...
export class AvlTree<K, V> implements AvlTreeApi<K, V> {
  ...
  public findEnclosingRange(key: K): Range<K> {
    ...
    return this._findEnclosingRange(key, this._root, undefined, undefined)
  }

  private _findEnclosingRange(key: K, root: Node<K, V> | null, minTs: K | undefined, maxTs: K | undefined): Range<K> {
    if (root === null) {
    ...
    if (result === 0) {
      ...
      return this._findEnclosingRange(key, root.left, minTs, root.key)
    } else {
      return this._findEnclosingRange(key, root.right, root.key, maxTs)
    }
  }
